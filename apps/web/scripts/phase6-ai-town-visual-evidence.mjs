...
const targets = [
  { label: "1440x1000", viewport: { width: 1440, height: 1000 }, states: ["default", "research", "reviewer", "handoff", "activity"] },
  { label: "1024x768", viewport: { width: 1024, height: 768 }, states: ["default", "research"] },
  { label: "390x844", viewport: { width: 390, height: 844 }, states: ["default", "research", "handoff"] },
  { label: "320x720", viewport: { width: 320, height: 720 }, states: ["default", "research"] },
];

function assert(condition, message) { if (!condition) throw new Error(message); }
async function capture(page, target, name) {
  await page.screenshot({ animations: "disabled", caret: "hide", fullPage: false, path: join(outputDir, `phase6-${target.label}-${name}.png`) });
}
async function assertNoOverflow(page, label) {
  ...
  assert(metrics.document <= metrics.viewport && metrics.body <= metrics.viewport, `${label}: horizontal overflow ${JSON.stringify(metrics)}`);
}
async function assertWorldDominant(page, target) {
  ...
  assert(box.width >= target.viewport.width * (mobile ? 0.92 : 0.64), `${target.label}: World is not horizontally dominant (${box.width}px)`);
  assert(box.height >= target.viewport.height * (mobile ? 0.68 : 0.56), `${target.label}: World is not vertically dominant (${box.height}px)`);
  assert((await page.getByLabel("Вид карты").count()) === 0, `${target.label}: rejected Phase 5 map selector is still visible`);
}
    ...
    try {
      ...
      if (target.states.includes("research")) {
        ...
        if (target.viewport.width <= 720) {
          ...
          assert(box && box.y > target.viewport.height * 0.35, `${target.label}: selected agent is not presented as a bottom sheet`);
          assert(box.width >= target.viewport.width - 24, `${target.label}: bottom sheet is too narrow`);
        }
        await capture(page, target, target.viewport.width <= 720 ? "world-research-lead-bottom-sheet" : "world-research-lead-selected");
      }
      ...
      if (target.states.includes("handoff")) {
        ...
        assert((await page.locator("[data-handoff-cue]").count()) === 1, `${target.label}: expected exactly one canonical handoff cue`);
      ...
      if (target.states.includes("activity")) {
        ...
        await page.getByText("Verify protocol contract", { exact: true }).first().waitFor({ state: "visible" });

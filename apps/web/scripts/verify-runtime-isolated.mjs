...
try {
  ...
  if (evidence.rows[0]?.migrations !== 24 || evidence.rows[0]?.revoked_sessions !== 1) {
    throw new Error("Standalone runtime did not preserve migration or revocation evidence");
  }

  process.stdout.write(
    `${JSON.stringify({ status: "PASS", postgresImage: POSTGRES_IMAGE, migrations: 24, authLifecycle: true, worldAuth: true, hubAuth: true, nativeChatProfileAuth: true, hubCommandAuth: true, hubCommandReplay: true, executionPreferenceAuth: true, executionPreferenceWrite: true, conversationAuth: true, agentConversationAuth: true, taskAuth: true, approvalAuth: true, restartRevocation: true, optionalAdapterIsolation: true })}\n`,
  );
} finally {

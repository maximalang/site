...
| 20 | Automatically choose the rational Route | PARTIAL | Resource Broker scores fresh quality, limits, cost, latency and load evidence and appends its exact decision event. Durable API_MODEL and LOCAL_MODEL Task adapters are now registered in production; Hub commands create provenance-validated execution routes and Agent binding/sessions; PostgreSQL proves paid-call idempotency, restart replay, result, usage, cost and memory projection. A live LiteLLM dispatch selected by the Broker plus fallback/failover evidence and one-step Hub provisioning UI remain the gap. |
...
1. Add one-step Hub provisioning for a model execution route, Agent binding and
   explicit conversation lifecycle without hidden or orphaned sessions.
2. Prove a Broker-selected live LiteLLM dispatch and bounded fallback/failover.
3. Add remaining approval-gated predefined integration mutations without
   exposing arbitrary remote commands.
4. Run the two account-bound live gates and repeat the full browser/runtime

...
| 18 | Manage MCP/n8n/servers/SSH from the site | PARTIAL | The owner-only Hub registry creates, lists, enables, disables and explicitly tests MCP, n8n, GitHub and SSH endpoints. A `READY` integration exposes a predefined read-only action: MCP `tools/list`, n8n workflow list, GitHub repository list or SSH host inspection. GitHub workflow dispatch and MCP `tools/call` now use separate durable request and explicit owner-decision commands. MCP tools must first be discovered and registered with immutable bounded arguments; callers submit only the PostgreSQL allowlist ID. PostgreSQL records `PENDING/DENIED/EXECUTING/terminal` state, resolves execution inputs before commit, and treats an unknown transport outcome as terminal without retry. Exact-host allowlisting, DNS pinning and encrypted/write-only credentials remain enforced. Real PostgreSQL, HTTP, component and executor tests prove the boundary. SSH service/deployment actions remain unavailable until an allowlisted service registry exists; arbitrary remote execution is intentionally not exposed. |

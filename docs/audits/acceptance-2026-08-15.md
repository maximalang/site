...
| 18 | Manage MCP/n8n/servers/SSH from the site | PARTIAL | The owner-only Hub registry creates, lists, enables, disables and explicitly tests MCP, n8n, GitHub and SSH endpoints. Protocol-aware probes use MCP initialize (JSON/SSE), n8n readiness, authenticated GitHub `/user`, or SSH banner only; exact-host allowlisting, reserved-address blocking, private-network acknowledgement, pinned DNS, no redirects, bounded output and durable PostgreSQL replay are enforced. Credentials remain encrypted/write-only and only a successful probe can set `READY`. Arbitrary execution adapters for n8n/GitHub/SSH are intentionally not exposed yet. |

...
- `AgentId`, `AccountId`, `ConversationId`, `MessageId`, `SessionId`, `RouteId` and other identities have
...
- `Conversation` is canonical continuity; replaceable runtime locators exist
  only in `ConversationSession` and never cross the owner intent boundary.
- Conversation messages distinguish owner/domain provenance from
  Agent/runtime provenance with a closed discriminated union.

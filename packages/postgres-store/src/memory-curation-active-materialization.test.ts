...
const ids = {
  ...
  requestedMemory: "context_item_55555555-5555-5555-5555-555555555555",
} as const;

const proposedAt = "2026-08-15T00:00:00.000Z";
const decidedAt = "2026-08-15T00:00:01.000Z";

describe("PostgresMemoryCurationStore active materialization fallback", () => {
  it("fails closed when an insert conflict has no memory active at the decision snapshot", async () => {
    const query = vi.fn(async (sql: string) => {
      ...
      throw new Error(`Unexpected query after failed materialization: ${sql}`);
    });
    const pool = { connect: vi.fn(async () => ({ query, release: vi.fn() })) };

    await expect(
      new PostgresMemoryCurationStore(pool as never, {
        contextItemId: () => ids.requestedMemory,
      }).decide({
        schemaVersion: 1,
        id: ids.decision,
